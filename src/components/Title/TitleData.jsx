import React from 'react';

export const TitleData = [
	{
		title: '02. Projects',
		body: <span></span>,
	},
	{
		title: '03. Skills',
		body: (
			<span>
				Proficient in languages and frameworks such as&nbsp;
				<mark>HTML, CSS, JavaScript, and React.</mark> My expertise extends to crafting
				scalable UI components with React, and I possess Back-End skills utilizing&nbsp;
				<mark>Axios and NodeJS.</mark> Ready to deliver practical and effective web
				solutions.
			</span>
		),
	},
	{
		title: 'VanSippy | Happy Hour Finder',
		body: (
			<span>
				Front and Back-end development using Yelp, Mapbox.js and Axios for API requests.
				Used of Material UI to provide a clean and responsive design. The app allows users
				to find happy hour deals in Vancouver.
			</span>
		),
	},
];
